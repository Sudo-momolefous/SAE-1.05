...
// ---------------- bouton menaces corrigé
function updateMenaces(){{
    let limit = parseInt(document.getElementById("limitMenaces").value);
    let m = document.getElementById("menaces");
    m.innerHTML = "";

    // trier les menaces par nombre de requêtes décroissant
    let sortedMenaces = Object.entries(menaces).sort((a,b)=>b[1]-a[1]);

    // ne garder que le top 'limit' éléments
    let entries = sortedMenaces.slice(0, limit);
