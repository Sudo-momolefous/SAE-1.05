...
# -----------------------------
# site web
def generer_site(ports, menaces):
    # genere html avec donnees integrees
...
<script>
let portsData = {json.dumps(ports)}
...
function dessiner(){{
    ...
    chart=new Chart(ctx,{{
        type:type,
        data:{{
            labels:Object.keys(portsData),
            datasets:[{{label:"nombre paquets par port",data:Object.values(portsData)}}]
        }}
    }})
}}
...
def lancer_analyse(chemin_fichier):
    ...
    generer_site(dict(compteur_ports), menaces)
    ouvrir_site()
